...
const tempo = (date: number, bpm: number): InstructionOptions<'tempo'> => ({
    id: `tempo_${date}`, date, bpm, beatLength: 0.25,
})
...
describe("an accentuation pattern's span", () => {
    ...
    test("four quarter-note beats reach one whole note", () => {
        ...
        expect(instructionsEffectiveAtDate(mpm, justInside, 'accentuationPattern', 'global'))
            .toHaveLength(1)
        expect(instructionsEffectiveAtDate(mpm, PULSES_PER_WHOLE, 'accentuationPattern', 'global'))
            .toHaveLength(0)
    })
    ...
    test("six eighth-note beats reach three quarters, not six", () => {
        ...
        expect(instructionsEffectiveAtDate(mpm, barIn68 - 1, 'accentuationPattern', 'global', 8))
            .toHaveLength(1)
        expect(instructionsEffectiveAtDate(mpm, barIn68, 'accentuationPattern', 'global', 8))
            .toHaveLength(0)

        // Read as quarters — the old, unconditional assumption — the same pattern claims twice
        // the span it has.
        expect(instructionsEffectiveAtDate(mpm, barIn68, 'accentuationPattern', 'global'))
            .toHaveLength(1)
    })
})
