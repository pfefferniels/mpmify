...
const attrs = (record: Record<string, unknown>) => Object.entries(record)
...
const element = (name: string, record: Record<string, unknown>, children = '') =>
    children
        ? `<${name}${attrs(record)}>${children}</${name}>`
        : `<${name}${attrs(record)}/>`
...
const articulationInstructions = (truth: ArticulationTruth, notes: ScoreNote[]) => {
    ...
    return notes
        .map((note, index) => element('articulation', {
            date: note.date,
...
export const truthMpm = (truth: Truth, notes: ScoreNote[] = []): string => {
    ...
    if (truth.tempo?.length) {
        maps.push(element('tempoMap', {}, truth.tempo
            .map((span, index) => element('tempo', {
                date: span.date,
    ...
    if (truth.rubato?.length) {
        maps.push(element('rubatoMap', {}, truth.rubato
            .map((frame, index) => element('rubato', {
                date: frame.date,
    ...
    if (truth.dynamics?.length) {
        maps.push(element('dynamicsMap', {}, truth.dynamics
            .map((span, index) => element('dynamics', {
                date: span.date,
    ...
    if (truth.accentuation) {
        ...
        styles.push(element('metricalAccentuationStyles', {}, element('styleDef', { name: STYLE },
            element('accentuationPatternDef', { name: pattern.name, length: pattern.length },
                pattern.accentuations
                    .map(accentuation => element('accentuation', {
                        beat: accentuation.beat,
                        value: accentuation.value,
                        'transition.to': accentuation['transition.to'],
                    }))
                    .join('')))))
        maps.push(element('metricalAccentuationMap', {},
    ...
    if (truth.articulation) {
        ...
        styles.push(element('articulationStyles', {}, element('styleDef', { name: STYLE },
            articulation.defs
                .map(def => element('articulationDef', { ...def }))
                .join(''))))
        maps.push(element('articulationMap', {},
            element('style', {
                date: 0,
                'name.ref': STYLE,
