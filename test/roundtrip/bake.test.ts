 ...
 * `mergeArgumentations` both called it without one. Every real `info.json` has `InsertPedal` in
...
describe('the bake', () => {
    test('turns the chain into segments', () => {
        // 84 calls in 20 argumentations become 12 segments of 71 spans: an argumentation whose
        // every element a later transformer removed drops out, and `mergeArgumentations` folds
        // the ones covering identical ticks together.
