...
describe('the structural checks have teeth', () => {
    ...
    test('a repeated xml:id is caught (#30)', () => {
        expect(checks(wrap('', '<tempoMap><tempo date="0" xml:id="t" bpm="60"/><tempo date="720" xml:id="t" bpm="90"/></tempoMap>')))
            .toContain('unique xml:id')
    })

    test('NaN written into an attribute is caught (#44)', () => {
        expect(checks(wrap('', '<tempoMap><tempo date="0" xml:id="t0" bpm="NaN"/></tempoMap>')))
            .toContain('attribute values are values')
    })

    test('a transition with no successor is caught (#24)', () => {
        expect(checks(wrap('', '<tempoMap><tempo date="0" xml:id="t0" bpm="60" transition.to="90"/></tempoMap>')))
