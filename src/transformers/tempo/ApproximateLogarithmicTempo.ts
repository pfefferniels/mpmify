...
function fitSegments(
    ...
    const objective = (candidateTau: number[], candidateShapes: number[]): number => {
        ...
        for (let k = 0; k < nSeg; k++) {
            total += segmentSse(
                segOnsets[k], candidateTau[k], candidateTau[k + 1],
                candidateShapes[k], spanTicks[k], beatLength
            );
        }
        return total;
    };
    ...
    for (let k = 0; k < nSeg; k++) {
        shapes[k] = optimizeShape(segOnsets[k], tau[k], tau[k + 1], spanTicks[k], beatLength, undefined);
    }
    ...
    for (let iter = 0; iter < MAX_ITER; iter++) {
        const moved = jointGaussNewtonStep(
            segOnsets, tau, tauInit, shapes, spanTicks, beatLength,
            segData, inferredDirections, objective
        );
        ...
        if (moved.tau < TAU_CONVERGENCE_BPM && moved.shape < SHAPE_CONVERGENCE) {
            if (!refreshShapes(segOnsets, tau, shapes, spanTicks, beatLength, segData, inferredDirections, objective)) break;
        }
    }
 ...
 */
function weightedLinearFit(data: DataPoint[]): { intercept: number, slope: number, slopeStdError: number } {
...
function optimizeShape(
    ...
    spanTicks: number, beatLength: number,
    ...
    const objective = (im: number) =>
        segmentSse(effective, tau0, tau1, im, spanTicks, beatLength);
    ...
    for (let g = 1; g <= SHAPE_GRID; g++) {
        ...
        if (val < bestVal) { bestVal = val; bestIm = im; bestIndex = g; }
    }
...
function inferSegmentDirections(segData: DataPoint[][]): TempoDirection[] {
    ...
    for (const data of segData) {
        if (data.length < 2) { directions.push('auto'); continue; }

        // x is normalised to [0,1], so the slope equals τ_right − τ_left.
        const { slope, slopeStdError } = weightedLinearFit(data);
        const confident = Math.abs(slope) >= MIN_INFERRED_DIRECTION_DELTA_BPM
...
function projectState(
    tau: number[], shapes: number[], segData: DataPoint[][], directions: TempoDirection[]
): void {
...
function jointGaussNewtonStep(
    ...
    for (let k = 0; k < nSeg; k++) {
        ...
        const t0 = tau[k], t1 = tau[k + 1], im = shapes[k];
        ...
        const partials: [Float64Array, Float64Array, number][] = [
            [segmentElapsed(onsets, t0 + epsTau0, t1, im, span, beatLength),
             segmentElapsed(onsets, t0 - epsTau0, t1, im, span, beatLength), 2 * epsTau0],
            ...
            pinned[k]
                ...
                : [segmentElapsed(onsets, t0, t1, imUp, span, beatLength),
                   segmentElapsed(onsets, t0, t1, imDown, span, beatLength), imUp - imDown],
            [segmentElapsed(onsets, t0, t1 + epsTau1, im, span, beatLength),
             segmentElapsed(onsets, t0, t1 - epsTau1, im, span, beatLength), 2 * epsTau1],
        ];
...
function refreshShapes(
    ...
    for (let k = 0; k < shapes.length; k++) {
        candidateShapes[k] = optimizeShape(
            segOnsets[k], tau[k], tau[k + 1], spanTicks[k], beatLength, shapes[k]
        );
    }
    projectState(candidateTau, candidateShapes, segData, directions);
