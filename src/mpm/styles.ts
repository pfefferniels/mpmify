 ...
 */
const styleDef = (
    mpm: Mpm, definitionType: DefinitionType, scope: Scope, create: boolean,
): AnyStyle | null => {
...
export const insertStyle = (
    mpm: Mpm, style: Style, instructionType: InstructionType, scope: Scope,
): Style => {
...
export const insertDefinition = <T extends DefinitionType>(
    mpm: Mpm, type: T, definition: DefOf<T>, scope: Scope,
): void => {
...
export const removeDefinition = <T extends DefinitionType>(
    mpm: Mpm, type: T, definition: DefOf<T>,
): void => {
    const collection = collectionNameOfKind(styleKinds[type])
    if (collection === null) return
